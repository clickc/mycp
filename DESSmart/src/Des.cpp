...
#define CHECK_MSG(x,msg)	{if( !(x) ){return true;}}
...
bool CDes::Encrypt(const char *OutFile,const char *InFile,const char *KeyStr)
{
	...
	FILE_CHECK( Encrypt(fh_out,fh_in,KeyStr,OutFile,InFile),OutFile )
...
bool CDes::Decrypt(const char *OutFile,const char *InFile,const char *KeyStr)
{
	...
	FILE_CHECK( Decrypt(fh_out,fh_in,KeyStr,OutFile,InFile),OutFile )
...
//  备注：当密钥长度>8时，系统自动使用3次DES加密
/******************************************************************************/
bool CDes::Encrypt(FILE &fh_out,FILE &fh_in,const char *KeyStr,const char *OutFile,const char *InFile)
{
	////CWindow  wnd;
	long     len,k=0,TBlock;
...
//	deshead.Ver = 1;
    // 文件长度信息
       deshead.TLen = nol_ll(InFile);
     printf("deshead.TLen is %d \n",deshead.TLen);
	// 计算总块数
    TBlock=(deshead.TLen+BUFSIZE-1)/BUFSIZE;
    printf("deshead.TLen is %ld \n", TBlock);
	// 加密密钥串(用于解密时验证密钥的正确性)
	memset(deskey,0,16);
	strcpy(deskey,KeyStr);
        printf("deskey is %s \n",deskey);
        printf("size deskey is %d \n",sizeof(deskey));
         //printf("deshead.DesKey is %s \n",deshead.DesKey);
	Encrypt(deshead.DesKey,deskey,16);
         printf("deshead.DesKey is 0x%x \n",deshead.DesKey);
	// 写入信息头
	fwrite((char*)&deshead,1,sizeof(deshead),fout);
    // 显示等待光标
	////wnd.ShowWaitCursor();

	// 读取明文到缓冲区
	while( (len=fread(databuf,1,BUFSIZE,fin)) >0 )
	{   // 显示加密进度
...
//  备注：当密钥长度>8时，系统自动使用3次DES解密
/******************************************************************************/
bool CDes::Decrypt(FILE &fh_out,FILE &fh_in,const char *KeyStr,const char *OutFile,const char *InFile)
{
    ////CWindow  wnd;
    long     len,k=0,TBlock;
    ...
    CHECK( SetSubKey(KeyStr) )
	// 读取信息头并检查长度
	CHECK_MSG(fread(&deshead,1,sizeof(deshead),fin) == sizeof(deshead),
	           "错误：该文件不是有效的DES加密文件!" )
    // 版本控制
...
int main(int argc,char *argv[])
{
  ...
  FILE fh_out;
  FILE fh_in;
  const char *KeyStr="blue";
  const char *OutFile="temp/out.des";
  const char *InFile="temp/out.txt"; 
  cs.Encrypt(fh_out,fh_in,KeyStr,OutFile,InFile);
