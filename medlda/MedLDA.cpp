...
int MedLDA::run_em(char* start, char* directory, Corpus* corpus, Params *param)
{
	...
	int d, n;
	double **var_gamma, **phi;//**var_gamma应该是二维矩阵,行是文档,列是主题,元素(i,j)是指文档i属于主题j的概率，在这里**var_gamma和**phi是LDA模型原分布的近似分布的参数，即变分参数
	...
	phi = (double**)malloc(sizeof(double*)*max_length);//在变分分布中，**phi是二维数组行是一篇文档中单词的位置，列是主题，元素(i,j)是指具体一篇文档中第i个单词属于主题j的概率，所以**phi的行数一般取所有文档长度的//最大值
...
bool MedLDA::mle(SuffStats* ss, Params *param, bool bInit /*= true*/)
{
	...
	bool bRes = true;
	printf("begin the svmStructSolver!");
	if ( !bInit ) {
		svmStructSolver(ss, param, m_dMu);
		printf("end the svmStructSolver!");
	} 
...
void MedLDA::svmStructSolver(SuffStats* ss, Params *param, double *res)
{
  ...
  // fprintf(fileptr, "structmodel[0]=%5.5f ", structmodel.w[0]);
	for ( int y=0; y<m_nLabelNum; y++ ) {
		for ( int k=0; k<m_nK; k++ ){
			int etaIx = y * m_nK + k;
...
SuffStats* MedLDA::new_suffstats( )
{
	...
	memset(ss->alpha_suffstats, 0, sizeof(double)*m_nK);//将alpha_suffstats的所有元素设置为0
...
void MedLDA::new_model(int num_docs, int num_terms, int num_topics, int num_labels, double C)
{
	...
	m_dEta = (double*)malloc(sizeof(double) * num_topics * num_labels);//Eta使用向量存储二维矩阵，行是主题，列是标记，元素
	//[i*num_labels + j]指主题i和标记j的转换概率
	m_dMu = (double*)malloc(sizeof(double) * num_docs * num_labels);//Mu使用向量存储二维矩阵，行是文档，列是标记，元素[i*num_labels + j]
	//指文档i和标记j之间的关系值
...
void MedLDA::predict(Document *doc, double **phi)
{
	doc->predlabel = -1;
	double dMaxScore = 0;
    printf("the number of  label(m_nLabelNum):%d",m_nLabelNum);	
	for ( int y=0; y<m_nLabelNum; y++ )
	{
		double dScore = 0;
		for ( int k=0; k<m_nK; k++ ) {
